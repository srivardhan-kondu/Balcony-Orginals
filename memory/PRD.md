...
- Premium scroll system (v7): CursorPreview — global cursor-follow project still (lerped, tilt by cursor velocity, viewfinder corners) on any [data-preview] element (upcoming rows home + slate page), desktop pointer:fine only; scroll-velocity skewY on <main> via Lenis velocity (film-slip feel, max ±3deg); ScrollFill — word-by-word ghost→bright fill on Final CTA + Why statement (framer useScroll per-word ranges); sticky stacking hero (sticky top-0, covered by opaque z-5 content wrapper, dims to 0.62 + scales 0.95 as covered); ReelModal portaled to document.body so main's transform can't trap it
