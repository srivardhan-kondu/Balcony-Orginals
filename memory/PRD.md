...
- Intro: REAL PRELOADER (v11) — 000→100 counter tracks actual loading (document.fonts 20%, poster image 15%, brand video readiness 50% via loadeddata/canplaythrough, 1.6s minimum rhythm 15%), hard caps at 5s/8s/18s so no asset can block the site; at 100 the brand video rolls from frame 0, tagline reveals, counter fades; skip/ESC always available; once per session (sessionStorage `bo-intro-seen`); video-stall fallback exits cleanly to the hero
...
- Shared sound system (v6): src/lib/sound.js — one lazily-created AudioContext + master gain + enabled flag with pub/sub; reel-whir and clap both route through it; reel-sound toggle persists across pages; playClap() (3 filtered noise pulses + low thump) fires exactly at the clapperboard snap, only when sound is enabled
...
- FilmRing v2 (v11, restored per user — takeover removed): bigger stage (470px panels, radius up to 760, 62vh container), floor reflections per panel (scaleY(-1) + mask), front-panel spotlight (per-frame brightness from facing angle), scroll-linked rotation on top of drag/inertia physics, reel-whir sound toggle back in place; ArchiveTakeover retired (file kept, unused)
