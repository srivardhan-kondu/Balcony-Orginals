...
- Intro: clean splash (v12, user-directed) — logo animation video only; counter, progress bar and preloader machinery removed; plays on open, fades on end (10s cap + error fallback), skip/ESC, once per session
...
- Page transitions (v5): ClapperTransition — every route change cuts to a black studio and marks a take on a 3D clapperboard (CSS perspective; camera element pushes in 2.8% across the take while the rig element carries attitude + impact shudder, so the two never fight for one transform), slate reads SCENE (page name) / TAKE (increments per navigation) / 24 FPS / date, then lifts to reveal the new page; 2s total on a fixed beat — arm lifts 35° at 0.5s with overshoot, falls in 120ms at 1.2s with a motion-blur smear, clap fires at 1.32s on impact (not on release), overlay clears at 1.56s; scene type steps down a register by name length instead of truncating; timers cleaned up on rapid navigation
- Shared sound system (v6): src/lib/sound.js — one lazily-created AudioContext + master gain + enabled flag with pub/sub; reel-whir and clap both route through it; reel-sound toggle persists across pages; playClap() (3 filtered noise pulses + low thump) fires exactly at the clapperboard impact, only when sound is enabled
- Premium scroll system (v7): CursorPreview — global cursor-follow project still (lerped, tilt by cursor velocity, viewfinder corners) on any [data-preview] element, desktop pointer:fine only; ScrollFill — word-by-word ghost→bright fill on Final CTA + Why statement; sticky stacking hero (dims 0.62 + scales 0.95 as covered); ReelModal portaled to document.body
- Scroll & 3D upgrade (v8, replaced velocity skew — user feedback): THE SCREENING ROOM — pinned sticky section, vertical scroll unspools all 5 projects horizontally (measured px overflow, progress hairline, "Full archive" end card); ProjectCard curtain-iris entrance (clipPath inset + rotateX 9° + y whileInView, transformPerspective 1000); FilmReel — chrome 3D film reel (disc, rim torus, hub, 5 dark lightening holes) bleeding off the right edge of the Upcoming section, spin linked to scroll; ProjectorBeam — additive volumetric cone from upper-right with 130 drifting dust motes, slow sweep, in the Gems section
- SlateFocus (v9, from user mockup): scroll-driven focus slate replaces upcoming rows on Home + Upcoming page — each project is an 88svh panel (numbered display title, meta, logline, status chip + View project button, 16:9 still); panel nearest viewport center goes bright/full-colour, others dim to 25% + grayscale; sticky progress rail (dots + filling spine); caption "Focus on one story at a time…"; cursor-preview removed from these rows (superseded)
- ArchiveTakeover (v10, replaced FilmRing per user): fullscreen Apple-style scroll takeover — tall section (n×110svh) + sticky 100svh stage; each scroll segment wipes the next story up over the last (clipPath inset wipe + image scale 1.18→1 + text rise); chrome UI: overline, 0N/05 counter via useMotionValueEvent, right progress ticks, sound toggle relocated here; sticky releases after final story, page continues; FilmRing retired from home (file kept, unused)
- FilmRing v2 (v11/v12, restored per user; takeover retired, file kept unused): panels scale with viewport, radius derived from panel width (r=w/0.65) fixing mobile/tablet overlap; floor reflections per panel; front-panel spotlight brightness; scroll-linked rotation + drag/inertia physics; reel-whir sound toggle
