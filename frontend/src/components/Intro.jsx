...
export const Intro = () => {
  ...
  useEffect(() => {
    ...
    const hardT = setTimeout(() => end(), 9500);
    const onKey = (e) => {
      if (e.key === "Escape") end();
    };
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 top-0 h-px bg-bone/10">
        <div className="bo-prog h-px origin-left bg-gold" />
      </div>
      ...
      <button
        ...
        onClick={end}
        className="absolute bottom-[clamp(30px,5vh,54px)] right-[clamp(26px,4vw,58px)] inline-flex items-center gap-2.5 rounded-sm border border-bone/20 bg-ink/50 px-4 py-2.5 text-[11px] uppercase tracking-[0.16em] text-bone/70 transition-colors duration-300 hover:border-gold hover:text-gold"
      >
        Skip intro
